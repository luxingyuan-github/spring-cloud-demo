...
public class LunarUtil {
	...
	SimpleDateFormat chineseDateFormat = new SimpleDateFormat("yyyyMMdd");
	...
	final private static int leapMonth(int y) {
		if (y - 1900 > lunarInfo.length) {
			return (int) (lunarInfo[lunarInfo.length - 1] & 0xf);
		} else {
	...
	public static void main(String[] args) {
		try {
			LunarUtil lunarUtil = new LunarUtil("20180830");
	 ...
	 */
	public LunarUtil(String ymd) throws ParseException {
		Calendar cal = Calendar.getInstance();
		try {
			cal.setTime(chineseDateFormat.parse(ymd));
		} catch (Exception e1) {
			e1.printStackTrace();
			System.out.println("农历转换时传入日期格式不正确，正确格式应为：yyyyMMdd");
			throw e1;
		}
		int yearCyl = 0;
		int monCyl = 0;
